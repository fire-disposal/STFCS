...
export const roomHandlers = {
	...
	join: async (payload: unknown, ctx: RpcContext) => {
		...
		const joinSuccess = ctx.roomManager.joinRoom(
			p.roomId,
			ctx.socket.id,
			ctx.playerId,
			ctx.playerName,
	...
	rejoin: async (payload: unknown, ctx: RpcContext) => {
		...
		const reconnectSuccess = room.reconnectPlayer(
			ctx.socket.id,
			ctx.playerId,
			ctx.playerName,
			avatar
	...
	action: async (payload: unknown, ctx: RpcContext) => {
		...
		switch (p.action) {
			...
			case "start":
				...
				ctx.state.appendLog(
					createBattleLogEvent("game_started", {
			...
			case "kick":
				...
				const kickedPlayer = ctx.state.getPlayer(p.targetId);
				// 将被踢玩家的 Token 所有权转移给房主（防止 Token 孤立）
				const curState = ctx.state.getState();
				const kickedTokenIds = Object.entries(curState.tokens)
					.filter(([_, t]) => t.runtime?.ownerId === p.targetId)
					.map(([id]) => id);
				for (const tokenId of kickedTokenIds) {
					ctx.state.updateToken(tokenId, "runtime/ownerId", ctx.playerId);
				}
				room.leavePlayer(p.targetId);
				ctx.state.removePlayer(p.targetId);
				ctx.state.appendLog(
					createBattleLogEvent("kick", {
						playerId: ctx.playerId,
						playerName: ctx.playerName,
						targetId: p.targetId,
						targetName: kickedPlayer?.nickname ?? p.targetId,
						transferredTokens: kickedTokenIds.length,
					})
				);
				return;
			case "transfer_host":
				...
				ctx.state.appendLog(
					createBattleLogEvent("host_changed", {
						playerId: ctx.playerId,
						playerName: ctx.playerName,
						newHostId: p.targetId,
						newHostName: ctx.state.getPlayer(p.targetId)?.nickname ?? p.targetId,
						previousHostDisconnected: false,
					})
				);
				return;
		}
