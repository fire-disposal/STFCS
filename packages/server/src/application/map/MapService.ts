import { GameMap, type MapConfig } from "../../domain/map/Map";
import type { TokenConfig, TokenType } from "../../domain/map/TokenEntity";
import type { CameraConfig } from "../../domain/map/Camera";
...
export type MapServiceResult<T> = Result<T>;
...
export class MapService {
	...
	initializeMap(config: MapConfig): MapServiceResult<GameMap> {
		try {
			...
			return { success: true, data: this._map };
		} catch (error) {
			return {
				success: false,
	...
	placeToken(command: PlaceTokenCommand): MapServiceResult<TokenConfig> {
		...
		try {
			...
			return { success: true, data: token.toDTO() };
		} catch (error) {
			return {
				success: false,
	...
	updateCamera(command: UpdateCameraCommand): MapServiceResult<CameraConfig> {
		...
		return { success: true, data: this._map.camera.toDTO() };
	}
