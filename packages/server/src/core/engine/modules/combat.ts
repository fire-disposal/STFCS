...
export function applyCombat(context: EngineContext): { newState: any; events: any[] } {
  ...
  // 计算武器指向目标的角度（统一渲染）
  const weaponHeading = calculateWeaponHeadingToTarget(
  ...
  const newAttackerRuntime = {
    ...ship.runtime,
    hasFired: true,
    weapons: updateWeaponAfterFire(ship.runtime.weapons, payload.weaponId, weaponHeading),
  };
 ...
 * @returns 武器世界角度
 */
function calculateWeaponHeadingToTarget(
  shipPosition: { x: number; y: number },
  targetPosition: { x: number; y: number }
): number {
  return angleBetween(shipPosition, targetPosition);
}

/**
 * 更新武器状态（开火后）
 * 设置 currentHeading 为指向目标的角度
 */
function updateWeaponAfterFire(weapons: any[], weaponId: string, currentHeading: number): any[] {
  if (!weapons) return weapons;

  return weapons.map(weapon => {
    if (weapon.mountId === weaponId) {
      return {
        ...setWeaponFired(weapon),
        currentHeading,
      };
    }
    return weapon;
