 ...
 * 渲染护盾受击效果
 */
export function renderShieldHitEffect(shield: ShieldInstanceState, hitAngle: number): Graphics {
	const graphics = new Graphics();
 ...
 * 渲染辐能条
 */
export function renderFluxBar(flux: FluxInstanceState, options: FluxRenderOptions = {}): Container {
	const container = new Container();
