 ...
 * - CAPITAL: 大型复合形状
 */
export function drawShipGeometry(graphics: Graphics, config: ShipGeometryConfig): void {
	const {
		size,
		collisionRadius,
 ...
 * 绘制武器射界几何体
 */
export function drawWeaponArcGeometry(graphics: Graphics, config: WeaponGeometryConfig): void {
 ...
 * 绘制护盾几何体
 */
export function drawShieldGeometry(graphics: Graphics, config: ShieldGeometryConfig): void {
...
export function drawHexagon(
	...
	for (let i = 0; i < 6; i++) {
		...
		points.push(new Point(x + Math.cos(angle) * radius, y + Math.sin(angle) * radius));
	}
