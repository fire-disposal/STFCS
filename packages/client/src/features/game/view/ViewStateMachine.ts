...
import type { LayerId, ViewChange, ViewState, ViewUpdatePlan } from "./types";
...
export class ViewStateMachine {
	...
	private applyChangeToState(change: ViewChange): void {
		switch (change.type) {
			...
			case "viewport": {
				this.state.viewport = {
					...this.state.viewport,
					...(change.value as Partial<ViewState["viewport"]>),
				};
				break;
			}
	 ...
	 * 订阅状态变更
	 */
	subscribe(callback: (state: ViewState, changes: ViewChange[]) => void): () => void {
