...
import type { FactionId, HullSizeValue, ShipDefinition } from "@vt/types";
...
export const ShipSelector: React.FC<ShipSelectorProps> = ({
	...
	const availableShips = useMemo(() => {
		...
		// 显示阵营专属舰船和通用舰船
		return ships.filter((ship) => !ship.faction || ship.faction === faction);
	}, [ships, faction]);
	...
	if (availableShips.length === 0) {
		return (
			<div style={styles.container}>
				<div style={styles.emptyState}>该阵营暂无可部署的舰船</div>
			</div>
		);
	}
		...
		<div style={styles.container}>
			<div style={styles.shipList}>
				{Object.entries(shipsBySize).map(([size, sizeShips]) => {
					...
					return (
						<div key={size}>
							<div style={styles.categoryTitle}>{hullSizeNames[size as HullSizeValue]}</div>
							{sizeShips.map((ship) => (
								<ShipCard
									key={ship.id}
									ship={ship}
	...
	return (
		...
		>
			...
			<div style={styles.shipInfo}>
				<div style={styles.shipName}>{ship.nameLocalized?.zh ?? ship.name}</div>
				<div style={styles.shipDetails}>{ship.variant && <span>变体: {ship.variant}</span>}</div>
			</div>

			{/* 部署点数 */}
			{ship.cost !== undefined && <div style={styles.shipCost}>{ship.cost} DP</div>}
		</div>
	);
};
