...
import { getAvailableShips, getShipHullSpec } from "@vt/rules";
	...
	return (
		...
		>
			...
			<div style={styles.shipCardFooter}>
				...
				{ship.hasShield && <span style={styles.hasShield}>🛡️ 护盾</span>}
			</div>
		</div>
	);
};
	...
	return (
		<div style={styles.panel}>
			...
			<div style={styles.phaseHeader}>
				...
				{isDM && <div style={styles.dmBadge}>DM 模式</div>}
			</div>
			...
			{currentPhase === "DEPLOYMENT" ? (
				<>
					...
					{selectedShip && (
						<div style={styles.deploymentOptions}>
							...
							<div style={styles.deployHint}>💡 点击地图上的位置来部署舰船</div>

							<button style={styles.deployButton} onClick={() => handleDeploy(0, 0)}>
								部署在中心 (0, 0)
							</button>
						</div>
					)}
					...
					<div style={styles.readySection}>
						...
						<div style={styles.readyHint}>
							{isReady ? "你已准备完毕，等待其他玩家" : "所有玩家准备后自动进入下一阶段"}
						</div>
					</div>
				</>
			) : (
				<div style={styles.phaseLocked}>
					...
					<div>部署阶段已结束</div>
					<div style={styles.lockedHint}>当前阶段：{currentPhase}</div>
				</div>
			)}
		</div>
	);
