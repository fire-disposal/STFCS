...
import { Rocket, Shield, Bomb, Zap, FastForward, Sparkles, ZoomIn, ZoomOut, RotateCcw } from 'lucide-react';
...
export const CommandDock: React.FC<CommandDockProps> = ({
  ...
  const commandGroups = useMemo(() => {
      ...
      const shipGroup: CommandGroup = {
        ...
        buttons: [
          ...
          },
          {
            ...
            label: '排散',
            shortcut: 'V',
            disabled: disabled || selectedShip.isShieldUp || (selectedShip.fluxHard + selectedShip.fluxSoft <= 0),
            variant: 'default',
            onClick: onVent,
          },
        ],
    ...
    if (playerRole === PlayerRole.DM) {
      const dmGroup: CommandGroup = {
        id: 'dm_commands',
        title: 'DM 命令',
        buttons: [
          {
    ...
    return groups;
  }, [selectedShip, playerRole, disabled, onMove, onToggleShield, onFire, onVent, onNextPhase, onCreateObject]);
            ...
            {group.buttons.map((btn) => (
              <button
                ...
                className={`game-layout__command-btn ${
                  ...
                  btn.variant === 'primary' ? 'game-layout__command-btn--primary' :
                  btn.variant === 'danger' ? 'game-layout__command-btn--danger' : ''
                }`}
