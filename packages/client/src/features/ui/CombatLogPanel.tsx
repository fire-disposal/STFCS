		...
		<div style={styles.panel}>
			...
			<div style={styles.header}>
				<div style={styles.title}>
					<FileText className="game-icon--sm game-icon--primary" />
					战斗日志
				...
				<div style={styles.headerActions}>
					...
					<button
						data-magnetic
						className="game-btn game-btn--small game-btn--ghost"
						onClick={handleClear}
						title="清空日志"
					>
						<Trash2 className="game-icon--xs" />
					</button>
					<button
						data-magnetic
						className="game-btn game-btn--small game-btn--ghost"
						...
						title="导出日志"
					>
						<Download className="game-icon--xs" />
					</button>
					<button style={styles.closeButton} onClick={onClose}>
						<X className="game-icon--sm" />
					</button>
				</div>
			</div>
				...
				{Object.entries(LOG_TYPE_LABELS).map(([type, label]) => (
					<button
						...
						style={{
							...
							color: filter.types.includes(type as LogType)
								? "#fff"
								: LOG_TYPE_COLORS[type as LogType],
						}}
			...
			{/* 日志列表 */}
			<div ref={scrollContainerRef} style={styles.logList} onScroll={handleScroll}>
							...
							>
								<div style={styles.logHeader}>
									<span style={styles.logLevelIcon}>
										<>
											{React.createElement(LOG_LEVEL_ICONS[log.level], {
												className: "game-icon--xs",
												style: { color: LOG_LEVEL_COLORS[log.level] },
											})}
										</>
									</span>
									<span style={styles.logType}>{LOG_TYPE_LABELS[log.type]}</span>
									<span style={styles.logTime}>{formatTime(log.timestamp)}</span>
									<span style={styles.logRound}>R{log.round}</span>
								</div>
