...
export const ViewRotationControl: React.FC<ViewRotationControlProps> = ({ selectedShip }) => {
		...
		<div style={styles.panel}>
			<div style={styles.section}>
				...
				<div style={styles.controlRow}>
					<span style={styles.valueDisplay}>{Math.round(viewRotation)}°</span>
					<button style={styles.button} onClick={() => rotateBy(-15)}>
						-15°
					</button>
					<button style={styles.button} onClick={() => rotateBy(-5)}>
						-5°
					</button>
					<button style={styles.button} onClick={() => rotateBy(5)}>
						+5°
			...
			{selectedShip && (
				<div style={styles.section}>
					...
					<div style={styles.shipInfo}>
						舰船朝向：
						{formatAngle(selectedShip.transform.heading, { mode: "math", showDecimal: true })}
					</div>
			...
			{!selectedShip && <div style={styles.shipInfo}>选择一艘舰船以启用对齐功能</div>}
		</div>
	);
};
