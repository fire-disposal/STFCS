...
import type { ShipState, WeaponSlot, PlayerRoleValue } from '@vt/contracts';
import { ClientCommand as CC, WeaponState, Faction, PlayerRole } from '@vt/contracts';
import { Rocket, Shield, Sparkles, Crosshair, Zap, Check, Bomb, Wind } from 'lucide-react';
...
export const ShipActionPanel: React.FC<ShipActionPanelProps> = ({
  ...
  const handleMove = useCallback(() => {
    ...
    onSendCommand(CC.CMD_MOVE_TOKEN, {
      shipId: ship.id, x: newX, y: newY, heading: newHeading,
      movementPlan: { phaseAForward: movePhaseA, phaseAStrafe: movePhaseAStrafe, turnAngle, phaseBForward: movePhaseB, phaseBStrafe: movePhaseBStrafe },
    });
    setMovePhaseA(0); setMovePhaseAStrafe(0); setTurnAngle(0); setMovePhaseB(0); setMovePhaseBStrafe(0);
  ...
  const handleToggleShield = useCallback(() => {
    ...
    onSendCommand(CC.CMD_TOGGLE_SHIELD, { shipId: ship.id, isActive: !ship.isShieldUp, orientation: ship.transform.heading });
  }, [ship, canToggleShield, onSendCommand]);

  const handleFire = useCallback(() => {
    if (!ship || !canFire || !selectedWeaponId || !selectedTargetId) return;
    onSendCommand(CC.CMD_FIRE_WEAPON, { attackerId: ship.id, weaponId: selectedWeaponId, targetId: selectedTargetId });
    setSelectedWeaponId(null); setSelectedTargetId(null);
  ...
  const getStatusMessage = () => {
    ...
    if (playerRole === PlayerRole.PLAYER && ship.ownerId !== playerSessionId) return { type: 'warning', text: '这不是你的舰船' };
    if (playerRole === PlayerRole.PLAYER && currentPhase !== 'PLAYER_TURN') return { type: 'warning', text: `当前是${phaseNames[currentPhase] || currentPhase}` };
  ...
  return (
    <div className="game-panel action-panel">
      ...
      {status && (
        <div className={`action-status action-status--${status.type}`}>{status.text}</div>
      )}
      ...
      <div className="game-section">
        ...
        <div className="action-slider">
          ...
          <input type="range" className="action-slider__input"
            min={-ship.maxSpeed * 2} max={ship.maxSpeed * 2} value={movePhaseA}
            onChange={(e) => setMovePhaseA(Number(e.target.value))} disabled={!canMove} />
        </div>
        ...
        <div className="action-slider">
          ...
          <input type="range" className="action-slider__input"
            min={-ship.maxSpeed} max={ship.maxSpeed} value={movePhaseAStrafe}
            onChange={(e) => setMovePhaseAStrafe(Number(e.target.value))} disabled={!canMove} />
        </div>
        ...
        <div className="action-slider">
          ...
          <input type="range" className="action-slider__input"
            min={-ship.maxTurnRate} max={ship.maxTurnRate} value={turnAngle}
            onChange={(e) => setTurnAngle(Number(e.target.value))} disabled={!canMove} />
        </div>
        ...
        <div className="action-slider">
          ...
          <input type="range" className="action-slider__input"
            min={-ship.maxSpeed * 2} max={ship.maxSpeed * 2} value={movePhaseB}
            onChange={(e) => setMovePhaseB(Number(e.target.value))} disabled={!canMove} />
        </div>
        ...
        <div className="action-slider">
          ...
          <input type="range" className="action-slider__input"
            min={-ship.maxSpeed} max={ship.maxSpeed} value={movePhaseBStrafe}
            onChange={(e) => setMovePhaseBStrafe(Number(e.target.value))} disabled={!canMove} />
        </div>

        <button data-magnetic className={`game-btn game-btn--primary game-btn--full`}
          onClick={handleMove} disabled={ship.hasMoved || !canMove}>
          {ship.hasMoved ? (
      ...
      <div className="game-section">
        ...
        <button data-magnetic className={`game-btn game-btn--full ${ship.isShieldUp ? 'game-btn--active' : ''}`}
          ...
          {availableWeapons.length > 0 ? (
            availableWeapons.map((weapon) => (
              <div key={weapon.mountId}
                ...
                <span className="action-weapon__name">{weapon.name || weapon.mountId.slice(-6)}</span>
                <span className="action-weapon__stats">伤害:{weapon.damage} 射程:{weapon.range}</span>
              </div>
            ))
          ) : (
            <div className="game-empty">{ship.weapons.size > 0 ? '武器冷却中...' : '无可用武器'}</div>
          )}
        </div>
          ...
          <div className="action-target-select">
            ...
            {availableTargets.map((target) => (
              <div key={target.id}
                ...
                <span className="action-target__faction">{target.faction === Faction.PLAYER ? '玩家' : '敌方'}</span>
              </div>
            ))}
          </div>
        )}

        <button data-magnetic className="game-btn game-btn--danger game-btn--full"
          onClick={handleFire} disabled={ship.hasFired || !canFire || !selectedWeaponId || !selectedTargetId}>
          {ship.hasFired ? (
      ...
      <div className="game-section">
        ...
        <button data-magnetic className="game-btn game-btn--warning game-btn--full"
          onClick={handleVent} disabled={!canVent}>
