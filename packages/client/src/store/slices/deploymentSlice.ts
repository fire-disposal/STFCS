...
export const selectFactionShips = (faction: FactionId) =>
	createSelector([selectDeployedShips], (ships) => ships[faction] ?? []);

export const selectFactionReady = (faction: FactionId) =>
	createSelector([selectReadyStatus], (status) => status[faction] ?? false);

export const selectAllFactionsReady = createSelector([selectReadyStatus], (status) =>
	Object.values(status).every((r) => r)
);
...
export const selectFactionDeploymentPoints = (faction: FactionId) =>
	createSelector([selectDeploymentPoints], (points) => points[faction] ?? { used: 0, max: 100 });

export const selectDeploymentZone = (faction: FactionId) =>
	createSelector([selectDeploymentZones], (zones) => zones[faction]);
