...
export const GameCanvas: React.FC<GameCanvasProps> = ({
	...
	const shipCacheRef = useRef<
		Map<string, { token: Graphics; label: Text; hpBar: Graphics; isSelected: boolean }>
	>(new Map());
	...
	useEffect(() => {
		...
		if (currentPhase && currentPhase !== "NONE" && currentPhase !== "COMPLETED" && !isAnimating) {
			...
			const remainingFuel = {
				forward:
					currentPhase === "PHASE_A" || currentPhase === "PHASE_C"
						? movementState[currentPhase.toLowerCase()]?.fuel?.forwardMax -
								movementState[currentPhase.toLowerCase()]?.fuel?.forwardUsed || 0
						: 0,
				strafe:
					currentPhase === "PHASE_A" || currentPhase === "PHASE_C"
						? movementState[currentPhase.toLowerCase()]?.fuel?.strafeMax -
								movementState[currentPhase.toLowerCase()]?.fuel?.strafeUsed || 0
						: 0,
				turn:
					currentPhase === "PHASE_B"
						? movementState.phaseB?.fuel?.turnMax - movementState.phaseB?.fuel?.turnUsed ||
							maxTurnRate
						: 0,
			};
			...
			// 获取已消耗燃料
			const usedFuel = {
				forward: movementState[currentPhase.toLowerCase()]?.fuel?.forwardUsed || 0,
				strafe: movementState[currentPhase.toLowerCase()]?.fuel?.strafeUsed || 0,
				turn: movementState.phaseB?.fuel?.turnUsed || 0,
			};
			...
			const predictedCommand = {
				forward: movementState[currentPhase.toLowerCase()]?.lastMove?.forward || 0,
				strafe: movementState[currentPhase.toLowerCase()]?.lastMove?.strafe || 0,
				turn: movementState.phaseB?.lastMove?.turn || 0,
			};
	...
	return (
		<div ref={hostRef} className="game-map-container">
			<Application
				resizeTo={hostRef}
				autoDensity
				antialias
