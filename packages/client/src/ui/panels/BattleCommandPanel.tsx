...
export const BattleCommandPanel: React.FC<BattleCommandPanelProps> = ({
	...
	const phaseA = useGameStore((state) => state.phaseA);
	const phaseB = useGameStore((state) => state.phaseB);
	const phaseC = useGameStore((state) => state.phaseC);
	const startMovementAction = useGameStore((state) => state.startMovement);
	...
	// 初始化移动状态
	useEffect(() => {
		if (selectedShip && movementPhase === "NONE") {
			startMovementAction(selectedShip.maxSpeed, selectedShip.maxTurnRate);
		}
	}, [selectedShip, movementPhase, startMovementAction]);

	// 同步服务端阶段状态
	useEffect(() => {
		if (!selectedShip) return;
		const serverPhase = selectedShip.movePhase;
		if (serverPhase && movementPhase !== "NONE") {
			if (movementPhase !== serverPhase) {
				setMovePhaseAction(serverPhase as MovementPhaseValue);
			}
		}
	}, [selectedShip?.movePhase, movementPhase, setMovePhaseAction]);

	// 当前阶段剩余燃料
	const remainingFuel = useMemo(() => {
		const phaseState = movementPhase === "PHASE_A" ? phaseA : movementPhase === "PHASE_B" ? phaseB : phaseC;
		return {
			forward: phaseState.fuel.forwardMax - phaseState.fuel.forwardUsed,
			strafe: phaseState.fuel.strafeMax - phaseState.fuel.strafeUsed,
			turn: phaseState.fuel.turnMax - phaseState.fuel.turnUsed,
		};
	}, [movementPhase, phaseA, phaseB, phaseC]);
	...
	const handleExecuteMove = useCallback(async () => {
		...
		try {
			...
			const phaseName = phase === "PHASE_A" ? "A" : phase === "PHASE_B" ? "B" : "C";
	...
	}, [selectedShip, movementPhase, forwardInput, strafeInput, turnInput, networkManager, executeMoveAction]);
	...
	const phaseComplete = selectedShip?.hasMoved || movementPhase === "NONE";
				...
				<div className="combat-panel">
					...
					<div className="weapon-list">
						{weapons.map((weapon, idx) => {
							const isReady = weapon.cooldownRemaining === 0;
							const isCooldown = weapon.cooldownRemaining > 0;
							const isEmpty = weapon.currentAmmo === 0 && weapon.maxAmmo > 0;
							return (
								<button
									key={weapon.mountId}
